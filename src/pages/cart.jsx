...
const token = localStorage.getItem("access-token")
  ...
  const handleIncrease = async(items) => {
    try {
  ...
  const handleDecrease = async (items) => {
    try {
      if (items.quantity > 1) {
        const item = await apiRequest({
          url: `/carts/${items._id}`,
  ...
  return (
    <div className="pt-20 max-w-screen-2xl xl:px-24 px-10 ">
      ...
      {cart?.length > 0 ? (
        <div>
          <div className="overflow-x-auto w-screen -mx-12">
              ...
              {cart?.map((items, i) => (
                <tbody key={i}>
                  <tr>
                    ...
                    <td className="font-medium">{items.name}</td>
          ...
          <div className="flex my-12 md:flex-row flex-col  justify-between items-start">
            <div className="w-1/2 space-y-3">
              ...
              <p>
                Name:
                <span className="font-medium italic">{user?.displayName}</span>
              </p>
              <p>
        ...
        <div>
          <h1 className="mt-10 mx-10 text-3xl font-semibold">
