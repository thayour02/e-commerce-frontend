...
export default function CheckoutForm({ price, cart }) {
  ...
  const handleSubmit = async (event) => {
    ...
    if (paymentIntent.status === "succeeded") {
      ...
      const paymentInfo = {
        ...
        menuItem: cart.map((item) => item._id),
      };
  ...
  return (
    <div className="flex flex-col  mx-4 md:flex-row justify-start items-start gap-8">
      ...
      <div
        className="md:w-1/3 w-full space-y-3 card bg-base-100 
      ...
      >
        ...
        <form onSubmit={handleSubmit}>
          ...
          <button
            type="submit"
            disabled={!stripe}
            className="btn w-full mt-5 bg-green-500 text-white"
          >
            Pay
          </button>
        </form>
        {cardError ? <p className="text-red-400 italics">{cardError}</p> : ""}
